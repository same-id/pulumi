...
func TestSecretsProviderFromState(t *testing.T) {
	...
	// Init - secret provider should live on
	{
		...
		e.SetBackend(e.LocalURL())
		e.RunCommand(
			"pulumi", "stack", "init", stackName, "--secrets-provider", provider,
		)
	...
	// "pulumi up" fails if config has different secrets provider
	{
		// Create passphrase secrets provider config
		ps := &workspace.ProjectStack{}
		ps.EncryptionSalt = "badsalt"
		d, err := yaml.Marshal(ps)
		assert.NoError(t, err)
		err = os.WriteFile(stackConfigFile, d, 0o600)
		assert.NoError(t, err)

		_, se := e.RunCommandExpectError("pulumi", "up", "--non-interactive", "--yes", "--skip-preview")
		assert.Equal(t, se, "error: getting stack configuration: get stack secrets manager: "+
